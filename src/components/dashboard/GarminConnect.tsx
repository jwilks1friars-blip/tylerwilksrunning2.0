...
export default function GarminConnect({ connection, connectedViaParam }: Props) {
  ...
  async function handleBackfill() {
    setSyncing(true)
    setSyncResult(null)
    const res = await fetch('/api/garmin/backfill', { method: 'POST' })
    const data = await res.json()
    setSyncing(false)
    setSyncResult(data.imported != null ? `Imported ${data.imported} activities` : 'Sync failed')
  }
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-3 mb-3">
              <div className="w-2 h-2 rounded-full" style={{ backgroundColor: '#009cdf' }} />
              <p className="text-sm font-medium" style={{ color: '#f5f2ee' }}>Garmin Connected</p>
            </div>
            <p className="text-xs" style={labelStyle}>User ID: {connection.garmin_user_id}</p>
            ...
            <p className="text-xs mt-3" style={labelStyle}>
              Activities sync automatically via Garmin Health API webhook.
            </p>
            {syncResult && (
          ...
          <div className="flex flex-col items-end gap-3 shrink-0">
            ...
            <button
              ...
              style={{ color: '#9c9895' }}
            >
              {syncing ? 'Syncing…' : 'Import 90 days'}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="p-5" style={cardStyle}>
      ...
      <p className="text-sm mb-5" style={{ color: '#e8e0d4' }}>
        Connect Garmin to sync activities directly from your Garmin device.
      </p>
      <p className="text-xs mb-4" style={{ color: '#6b6560' }}>
        Requires Garmin Health API credentials. Contact your coach to set this up.
      </p>
      <a
        ...
        style={{ backgroundColor: '#009cdf', color: '#fff', borderRadius: '2px' }}
      >
        Connect Garmin
      </a>
    </div>
